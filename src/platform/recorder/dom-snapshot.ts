...
import { MAX_BODY_CHARS } from "../../core/recorder/cdp";
...
export async function captureDomSnapshot(tabId: number): Promise<CapturedEntry | undefined> {
  try {
    ...
    console.info(`[recorder] DOM snapshot ${result.html.length} chars from ${result.url}`);
    return {
      url: result.url,
      method: "DOM",
      status: 200,
      contentType: "text/html",
      responseBody: result.html.slice(0, MAX_BODY_CHARS),
    };
  } catch (error) {
