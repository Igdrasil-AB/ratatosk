import { findDocLinks, inferRecipe } from "../../core/recorder/infer";
import { buildAgentReport } from "../../core/recorder/report";
...
export interface StopResult {
  ...
  captured: number;
  /** A few captured requests (`status contentType url`) for quick eyeballing. */
  samples: string[];
  /** Invoice/receipt links found in the page HTML — a hint when no draft was made. */
  docLinks: string[];
  /** One paste-ready block (recipe + notes + samples + HTML excerpt) for a coding agent. */
  report: string;
}

export async function startRecording(tabId: number, url: string, mode: RecordMode): Promise<void> {
  console.info(`[recorder] startRecording mode=${mode} tab=${tabId} ${url}`);
...
export async function stopRecording(): Promise<StopResult> {
  ...
  if (docLinks.length) console.info(`[recorder] document links found: ${docLinks.slice(0, 8).join(", ")}`);
