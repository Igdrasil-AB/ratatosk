...
describe("Studio release publication", () => {
  it("keeps the checksum portable in a flat release-download directory", () => {
    ...
    const verified = spawnSync("shasum", ["-a", "256", "-c", "ratatosk-studio-v0.8.30.zip.sha256"], {
      cwd: fixture.remote,
      encoding: "utf8",
    });
  ...
  it("creates a missing release and verifies an existing release without replacing assets", () => {
    ...
    expect(readFileSync(fixture.log, "utf8")).toContain("release create v0.8.30");

    writeFileSync(fixture.log, "");
    const rerun = runPublisher(fixture, true);
    expect(rerun.status).toBe(0);
    expect(readFileSync(fixture.log, "utf8")).toContain("release download v0.8.30");
    expect(readFileSync(fixture.log, "utf8")).not.toContain("release create");
  });

  it("fails closed when an existing release asset differs", () => {
    const fixture = releaseFixture();
    writeFileSync(join(fixture.remote, "ratatosk-studio-v0.8.30.zip"), "different artifact");
  ...
  it("blocks an unchecked extra ZIP or a missing checksum before calling GitHub", () => {
    ...
    unlinkSync(join(missing.artifacts, "ratatosk-studio-v0.8.30.zip.sha256"));
...
function releaseFixture() {
  ...
  mkdirSync(artifacts);
  mkdirSync(remote);
  const archive = join(artifacts, "ratatosk-studio-v0.8.30.zip");
  ...
  writeFileSync(join(remote, "ratatosk-studio-v0.8.30.zip"), "reviewed artifact");
  writeFileSync(join(remote, "ratatosk-studio-v0.8.30.zip.sha256"), checksum);
