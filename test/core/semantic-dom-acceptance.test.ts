...
describe("live acquisition release acceptance", () => {
  ...
  it("accepts only the exact runtime, artifact, three-family, destination-readback matrix", () => {
    ...
    expect(parseSemanticDomAcceptanceReceipt(receipt(), "0.8.65", 54, 6, artifactSha256)).toMatchObject({
      collectorVersion: "0.8.65",
      discoveryRevision: 54,
  ...
  it("rejects stale identities, incomplete breadth, duplicate effects, and private fields", () => {
    vi.setSystemTime(new Date("2026-08-26T10:00:00.000Z"));
    expect(() => parseSemanticDomAcceptanceReceipt(receipt(), "0.8.64", 54, 6, artifactSha256)).toThrow(/0\.8\.64/);
    expect(() => parseSemanticDomAcceptanceReceipt(receipt(), "0.8.65", 55, 6, artifactSha256)).toThrow(/discovery revision 55/);

    const anotherBuild = receipt();
    anotherBuild.artifactSha256 = "b".repeat(64);
    expect(() => parseSemanticDomAcceptanceReceipt(anotherBuild, "0.8.65", 54, 6, artifactSha256)).toThrow(/artifact SHA-256/);

    const missingFamily = receipt();
    missingFamily.cases[1].family = "opaque_semantic_spa" as never;
    expect(() => parseSemanticDomAcceptanceReceipt(missingFamily, "0.8.65", 54, 6, artifactSha256)).toThrow(/server_rendered_documents/);

    const noIgdrasil = receipt();
    noIgdrasil.cases.forEach((entry) => { entry.destinationKind = "filesystem"; });
    expect(() => parseSemanticDomAcceptanceReceipt(noIgdrasil, "0.8.65", 54, 6, artifactSha256)).toThrow(/Igdrasil/);

    const noClickUp = receipt();
    noClickUp.clickupAccepted = false as never;
    expect(() => parseSemanticDomAcceptanceReceipt(noClickUp, "0.8.65", 54, 6, artifactSha256)).toThrow(/ClickUp/);

    const repeated = receipt();
    repeated.cases[0].cadenceRunActionCount = 1 as never;
    expect(() => parseSemanticDomAcceptanceReceipt(repeated, "0.8.65", 54, 6, artifactSha256)).toThrow(/idempotent/);

    const pageDownload = receipt();
    pageDownload.cases[0].pageOwnedDownloadDelta = 1 as never;
    expect(() => parseSemanticDomAcceptanceReceipt(pageDownload, "0.8.65", 54, 6, artifactSha256)).toThrow(/idempotent/);

    const wrongReadback = receipt();
    wrongReadback.cases[0].destinationReadbackCount = 0;
    expect(() => parseSemanticDomAcceptanceReceipt(wrongReadback, "0.8.65", 54, 6, artifactSha256)).toThrow(/idempotent/);
    ...
    expect(() => parseSemanticDomAcceptanceReceipt(sensitive, "0.8.65", 54, 6, artifactSha256)).toThrow(/unapproved/);
    vi.useRealTimers();
  });
});
