...
After the hostname-only wrapper records three passing families, run
`npm run build:live-acceptance-receipt`. It validates the closed TSV and writes
the ignored exact-artifact receipt; do not hand-edit or substitute synthetic
results. `npm run
...
families, Igdrasil readback, first-run acceptance/ledger agreement, zero
