...
export function buildDiscoveryIssueReport(diagnostic: DiscoveryDiagnosticV1): IssueReport {
  ...
  const slowest = [...diagnostic.attempts]
    ...
    .slice(0, 8)
    .map((attempt) =>
      `| ${attempt.page} | \`${attempt.route}\` | ${attempt.source} | ${attempt.adapter ?? "—"} | ${attempt.result}${attempt.probeCause ? `/${attempt.probeCause}` : ""}${attempt.replay?.firstFailure ? `@${attempt.replay.firstFailure.phase}/${attempt.replay.firstFailure.result}` : ""} | ${Math.round(attempt.durationMs)}ms |`)
    .join("\n");
